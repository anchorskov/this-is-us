...
ALTER TABLE hot_topic_civic_items ADD COLUMN match_score REAL;

-- Add matched_terms_json column
-- Stores JSON array of terms that matched between bill and topic
ALTER TABLE hot_topic_civic_items ADD COLUMN matched_terms_json TEXT;

-- Add excerpt column
-- Stores relevant excerpt from the bill text showing the match context
ALTER TABLE hot_topic_civic_items ADD COLUMN excerpt TEXT;
...
CREATE INDEX IF NOT EXISTS idx_hot_topic_matches_topic_score ON hot_topic_civic_items(topic_id, match_score DESC);
