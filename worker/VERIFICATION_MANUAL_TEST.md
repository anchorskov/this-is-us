...
- [ ] `./scripts/wr dev --local` is running from `/home/anchor/projects/this-is-us/worker`
...
| Issue | Check |
|-------|-------|
| Table doesn't exist | Run migration 0019 again: `./scripts/wr d1 execute WY_DB --file=migrations_wy/0019_create_civic_item_verification.sql --local` |
| /api/internal/civic/verify-bill returns 404 | Ensure bill ID exists: `./scripts/wr d1 execute WY_DB --command "SELECT COUNT(*) FROM civic_items;" --local` |
...
| Topic metadata not loading | Check EVENTS_DB for hot_topics table: `./scripts/wr d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM hot_topics WHERE is_active=1;" --local` |
