# summarize-logic.py – v3.7 (HTML Stylesheet Analysis)
"""
...
* **New HTML Stylesheet Href Detection**: Extracts `link href="..."` from HTML files to list external CSS dependencies.
* **New Audit Sections**:
    ...
    * "External Stylesheet Includes" for a consolidated view of CSS files loaded by HTML.
...
RE_HTML_LINK_HREF = re.compile(r'<link\s+[^>]*?href=["\'](.+?)["\']', re.MULTILINE) # New regex for CSS links
...
def analyze_file_code(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        results['html_link_href'] = ['(unreadable)'] # Initialize if error occurs
    ...
    results['html_link_href'] = sorted(list(set(results['html_link_href']))) # Sort and unique this list
    ...
    for root, dirs, files in os.walk(base):
        ...
        for fname in files:
            ...
            if fpath.suffix.lower() in TARGET_EXTENSIONS:
                ...
                if analysis['html_link_href']: # New section for HTML link hrefs
                    capture.append("\n**HTML Stylesheet Hrefs**:\n" + '\n'.join(f"- `{href}`" for href in analysis['html_link_href']))
...
def audit_html_link_href(capture: List[str]):
    """Audits and lists detected external stylesheet (link href) sources in HTML files."""
    capture.append("\n## 🎨 External Stylesheet Includes\n")
    all_link_hrefs: List[str] = []
    
    # Check all HTML files in layouts and content (as they might contain direct link tags)
    dirs_to_check = [Path('layouts'), Path('content')]
    for check_dir in dirs_to_check:
        if not check_dir.exists():
            continue
        for root, _, files in os.walk(check_dir):
            for fname in files:
                ...
                if fpath.suffix == '.html':
                    analysis = analyze_file_code(fpath)
                    if analysis['html_link_href']:
                        all_link_hrefs.extend([
                            f"`{href}` in `{fpath.relative_to(BASE_DIR)}`"
                            for href in analysis['html_link_href']
                        ])
    
    if all_link_hrefs:
        for href_path in sorted(list(set(all_link_hrefs))):
            capture.append(f"- {href_path}")
    else:
        capture.append(colour("No external stylesheet `href` attributes detected in HTML files.", YELLOW))
...
def main():
    ...
    capture: List[str] = ["# Logic Index – This Is Us Project (v3.7)\n"]

    if not args.townhall_only and not args.skip_index:
        walk_directory(BASE_DIR, capture)

    audit_townhall(capture)
    audit_api_endpoints(capture)
    audit_hugo_partials(capture)
    audit_html_script_sources(capture)
    audit_html_link_href(capture) # New audit section for HTML link hrefs
