...
- [ ] **Backup**: Create backup of EVENTS_DB
  ...
  wrangler d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM user_topic_prefs;" --json
  ```
  
- [ ] **Verify Current State**:
  ```bash
  # Check topic_index exists
  wrangler d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM topic_index;" --json
  
  # Check user_topic_prefs count
  wrangler d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM user_topic_prefs;" --json
  
  # Check hot_topics exists
  wrangler d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM hot_topics;" --json
  ```
...
- [ ] **Verify Migration Success**:
  ...
  wrangler d1 execute EVENTS_DB --command "PRAGMA table_info(user_topic_prefs);" --json
  
  # Check row count (should be similar to before)
  wrangler d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM user_topic_prefs;" --json
  
  # Check FK integrity
  wrangler d1 execute EVENTS_DB --command "SELECT COUNT(*) FROM user_topic_prefs WHERE topic_id NOT IN (SELECT id FROM hot_topics);" --json
