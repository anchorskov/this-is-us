...
LOCAL_EVENTS=$(npx wrangler d1 execute EVENTS_DB --local --command "SELECT COUNT(*) as migration_count FROM d1_migrations WHERE name LIKE '001%';" --json 2>&1 | jq -r '.[0].results[0].migration_count')
echo "  ✅ EVENTS_DB migrations applied: $LOCAL_EVENTS"

LOCAL_WY=$(npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as migration_count FROM d1_migrations WHERE name LIKE '000%';" --json 2>&1 | jq -r '.[0].results[0].migration_count')
...
# Check tables exist
CIVIC_ITEMS=$(npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM sqlite_master WHERE type='table' AND name='civic_items';" --json 2>&1 | jq -r '.[0].results[0].count')
[ "$CIVIC_ITEMS" = "1" ] && echo "  ✅ civic_items table exists" || echo "  ❌ civic_items table MISSING"

AI_TAGS=$(npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM sqlite_master WHERE type='table' AND name='civic_item_ai_tags';" --json 2>&1 | jq -r '.[0].results[0].count')
[ "$AI_TAGS" = "1" ] && echo "  ✅ civic_item_ai_tags table exists" || echo "  ❌ civic_item_ai_tags table MISSING"

HOT_TOPICS=$(npx wrangler d1 execute EVENTS_DB --local --command "SELECT COUNT(*) as count FROM hot_topics;" --json 2>&1 | jq -r '.[0].results[0].count')
echo "  ✅ hot_topics table: $HOT_TOPICS topics loaded"

HOT_CIVIC=$(npx wrangler d1 execute EVENTS_DB --local --command "SELECT COUNT(*) as count FROM sqlite_master WHERE type='table' AND name='hot_topic_civic_items';" --json 2>&1 | jq -r '.[0].results[0].count')
[ "$HOT_CIVIC" = "1" ] && echo "  ✅ hot_topic_civic_items table exists" || echo "  ❌ hot_topic_civic_items table MISSING"

echo ""
echo "✓ Checking data..."
PENDING=$(npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM civic_items WHERE status IN ('introduced', 'in_committee', 'pending_vote');" --json 2>&1 | jq -r '.[0].results[0].count')
echo "  ✅ Pending bills available: $PENDING"

LINKED=$(npx wrangler d1 execute EVENTS_DB --local --command "SELECT COUNT(DISTINCT topic_id) as count FROM hot_topic_civic_items;" --json 2>&1 | jq -r '.[0].results[0].count')
...
echo "   cd $WORKER_DIR && npx wrangler dev --local"
echo ""
