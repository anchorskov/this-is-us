...
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='table' AND name='bill_sponsors';"

# Expected output:
# { "results": [{"name": "bill_sponsors"}] }

# Query to verify wy_legislators table exists
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='table' AND name='wy_legislators';"
...
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='bill_sponsors';"
...
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='wy_legislators';"
...
npx wrangler d1 execute WY_DB --local --query "SELECT COUNT(*) as table_count FROM sqlite_master WHERE type='table' AND name IN ('bill_sponsors', 'wy_legislators');"

# 4. Check indices
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='index' ORDER BY tbl_name;"
...
- **Solution**: Verify all CREATE INDEX statements executed; check wrangler.toml for migrations_dir config
