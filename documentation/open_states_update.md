...
│   └── wrangler.toml          # Update bindings/vars
...
cd worker
wrangler d1 execute DB --file ../sql/0010_openstates_init.sql
wrangler d1 execute DB --file ../sql/0011_openstates_indexes.sql

cd ..
echo "=== Ingesting session $SESSION ==="
curl -X POST "http://127.0.0.1:8787/api/openstates/ingest/Wyoming/$SESSION"

wrangler d1 execute DB --command "SELECT COUNT(*) AS count FROM bills;"
wrangler d1 execute DB --command "SELECT COUNT(*) AS count FROM bill_actions;"
wrangler d1 execute DB --command "SELECT COUNT(*) AS count FROM bill_snapshots;"
```
...
wrangler d1 execute DB --command "SELECT COUNT(*) FROM bill_snapshots WHERE created_at > datetime('now', '-1 hour');"
```
...
wrangler secret put OPENSTATES_API_KEY   # once
wrangler dev --local
...
- In `wrangler.toml`, add a cron trigger (e.g., every 6 hours) and in the Worker’s `scheduled()` handler call `handleIngest` for the active session
...
4. Run `wrangler dev --local`
5. Apply migrations locally (`wrangler d1 execute DB --local --file sql/0010_openstates_init.sql`)
6. Run `scripts/openstates-seed.sh`
7. Inspect data in D1 (`wrangler d1 execute DB --command "SELECT * FROM bills LIMIT 1"`)
...
│   └── wrangler.toml                      # updated vars/cron
├── sql/
