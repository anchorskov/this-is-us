...
```bash
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('bill_sponsors', 'wy_legislators') ORDER BY name;"
```
...
```bash
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='bill_sponsors' ORDER BY name;"
```
...
```bash
npx wrangler d1 execute WY_DB --local --query "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='wy_legislators' ORDER BY name;"
```
...
```bash
npx wrangler d1 execute WY_DB --local --query "PRAGMA table_info(bill_sponsors);"
```
...
```bash
npx wrangler d1 execute WY_DB --local --query "PRAGMA table_info(wy_legislators);"
```
...
cd /home/anchor/projects/this-is-us/worker && \
npx wrangler d1 migrations apply WY_DB --local && \
npx wrangler d1 execute WY_DB --local --query "SELECT 'bill_sponsors' as table_name, COUNT(*) as column_count FROM pragma_table_info('bill_sponsors') UNION ALL SELECT 'wy_legislators', COUNT(*) FROM pragma_table_info('wy_legislators');"
```
...
**Solution**: This is normal. Run `npx wrangler d1 migrations list WY_DB --local` to verify:
```bash
npx wrangler d1 migrations list WY_DB --local
```
...
```bash
npx wrangler d1 migrations rollback WY_DB --local
npx wrangler d1 migrations apply WY_DB --local
```
