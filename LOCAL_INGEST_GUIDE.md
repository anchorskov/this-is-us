...
```bash
cd /home/anchor/projects/this-is-us/worker
npx wrangler d1 execute WY_DB --local --command "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name;" | head -20
```
...
```bash
cd /home/anchor/projects/this-is-us/worker
npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM civic_items WHERE source='lso';"
```
...
```bash
cd /home/anchor/projects/this-is-us/worker
npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM bill_sponsors;"
```
...
```bash
cd /home/anchor/projects/this-is-us/worker
npx wrangler d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM wy_legislators;"
```
...
- [ ] Migrations applied (`npx wrangler d1 migrations apply WY_DB --local`)
