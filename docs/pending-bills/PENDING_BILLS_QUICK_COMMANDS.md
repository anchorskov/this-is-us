...
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) as tables FROM sqlite_master WHERE type='table';"

# 3. Ingest bills from LSO
curl -X POST "http://127.0.0.1:8787/api/dev/lso/hydrate-bills?year=2025&limit=25"

# 4. Check counts
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) FROM civic_items WHERE source='lso';"
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) FROM bill_sponsors;"
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) FROM wy_legislators;"
...
```bash
# Check all tables
./scripts/wr d1 execute WY_DB --local --command ".schema" | grep "CREATE TABLE"
...
# View worker logs
tail -f /tmp/./scripts/wr.log  # if started in background
```
