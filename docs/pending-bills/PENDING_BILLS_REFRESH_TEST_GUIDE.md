...
./scripts/wr d1 execute WY_DB --local --command "DELETE FROM civic_items WHERE source='openstates';"
...
```bash
cd /home/anchor/projects/this-is-us/worker
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) FROM civic_items WHERE source='openstates';"
# Expected: > 0
```
...
### Config
- ✅ `worker/./scripts/wr.toml` - Cron schedule
...
**Solutions:**
- Reset verification: `./scripts/wr d1 execute WY_DB --local --command "DELETE FROM civic_item_verification WHERE check_type='lso_hydration';"`
- Retry scan: `SCANS=3 ./worker/scripts/admin-refresh-pending-bills.sh`

### Worker not responding

**Check:** Terminal 1 shows ./scripts/wr started
```
🚀 Starting ./scripts/wr on http://127.0.0.1:8787
```

**Restart:**
```bash
pkill -f "./scripts/wr dev"
./start_wrangler.sh
```
...
- [x] Cron configuration in ./scripts/wr.toml
...
2. **Verify cron is configured**
   - Check `worker/./scripts/wr.toml` has `[env.production.triggers]` with cron
...
4. **Monitor execution**
   ```bash
   ./scripts/wr tail  # Watch production logs
   ```
