...
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) as count FROM civic_items;"
```
...
./scripts/wr d1 execute WY_DB --local --command "SELECT COUNT(*) FROM civic_items;" 2>/dev/null | tail -1
