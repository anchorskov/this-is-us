...
5. **`scripts/d1-backup.sh`:**  
   “Write a bash script that exports all `EVENTS_DB` tables (events, candidates, townhall_posts, topic_index, user_topic_prefs, topic_requests, user_preferences) into timestamped `.sql` files using `./scripts/wr d1 execute … .dump`.”
...
4. Set up automated nightly `./scripts/wr d1 backup` into Cloudflare R2 or Git LFS for traceability.
