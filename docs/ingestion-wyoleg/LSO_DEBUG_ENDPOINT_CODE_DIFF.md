# Code Diff - LSO Service Debug Endpoint

## File 1: worker/src/routes/debugLsoService.mjs (NEW)
...
## File 2: worker/src/index.mjs (DIFF)

### Import Section (added ~line 60)
```diff
...
### Route Registration Section (added ~line 159)
```diff
...
|------|------|-------|--------|
| worker/src/routes/debugLsoService.mjs | NEW | 290 | Complete new file for debug handler |
| worker/src/index.mjs | UPDATE | 2 | Import the new handler |
| worker/src/index.mjs | UPDATE | 3 | Register the new route |

**Total Impact:**
- +290 lines (new file)
- +5 lines (imports + route registration)
- 0 breaking changes
- 0 dependencies added
- Backward compatible (adds new endpoint, doesn't modify existing code)
...
Test 2: 2025 bills
  year: "2025"
  total_items_received: 555
  total_items_before_filter: 724
  notes: ["Filtered 724 items to 555 by year field"]

Test 3: Field names (15 fields available)
  [amendments, billNum, billStatus, billType, chapterNo, effectiveDate, 
   enrolledNo, lastAction, lastActionDate, shortTitle, signedDate, 
   specialSessionValue, sponsor, substituteBills, year]
...
```bash
# Basic call with default limit
curl -s "http://127.0.0.1:8787/api/internal/debug/lso/billinformation-sample?year=2026" | jq .

# With custom limit
curl -s "http://127.0.0.1:8787/api/internal/debug/lso/billinformation-sample?year=2025&limit=5" | jq .

# With auth token (for remote)
curl -s "https://example.com/api/internal/debug/lso/billinformation-sample?year=2026" \
  -H "X-Internal-Token: your_secret" | jq .
```
