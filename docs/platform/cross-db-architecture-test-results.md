...
./scripts/wr dev --local --env preview
...
./scripts/wr d1 migrations apply EVENTS_DB --local --env preview

# WY_DB migrations (0001-0008)
./scripts/wr d1 migrations apply WY_DB --local --env preview
```
...
```bash
./scripts/wr d1 execute WY_DB --local --env preview --command \
...
```bash
./scripts/wr d1 execute EVENTS_DB --local --env preview --command \
