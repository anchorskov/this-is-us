...
### ✅ Local Database (`../scripts/wr/state/v3/d1`)
...
**Verification:**
```bash
./scripts/wr d1 execute WY_DB --local --command "PRAGMA table_info(civic_item_ai_tags);" --json
```
...
1. **0009_add_civic_item_ai_tags.sql** – Created table structure
   ```bash
   ./scripts/wr d1 execute WY_DB --remote --command "CREATE TABLE IF NOT EXISTS civic_item_ai_tags(...)"
...
2. **0010_add_reason_summary_to_civic_item_ai_tags.sql** – Added reason_summary column
   ```bash
   ./scripts/wr d1 execute WY_DB --remote --command "ALTER TABLE civic_item_ai_tags ADD COLUMN reason_summary TEXT;"
   ```
   **Status:** ✅ Executed and recorded in d1_migrations

**Verification:**
```bash
./scripts/wr d1 execute WY_DB --remote --command "PRAGMA table_info(civic_item_ai_tags);" --json
```
