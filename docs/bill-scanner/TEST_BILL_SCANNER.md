...
# Check ./scripts/wr (should be in worker/)
...
sqlite3 ../scripts/wr/state/v3/d1/miniflare-D1DatabaseObject/*/d1.sqlite \
...
### Terminal 1: Launch ./scripts/wr dev
```bash
cd /home/anchor/projects/this-is-us/worker
...
**Fix:**
```bash
# In worker/ directory, check ./scripts/wr.toml or .env
echo "OPENAI_API_KEY=$OPENAI_API_KEY"

# If not set, add to terminal before starting ./scripts/wr dev
...
```bash
# In ./scripts/wr dev logs, look for:
...
| Issue | Check | Fix |
|-------|-------|-----|
| 403 Scanner disabled | `BILL_SCANNER_ENABLED=true` | Set env var before `./scripts/wr dev` |
| 403 Forbidden (host) | curl uses 127.0.0.1 | Replace localhost with 127.0.0.1 |
| Empty results | OPENAI_API_KEY set | Export key before starting ./scripts/wr |
...
✅ No errors in ./scripts/wr dev console logs  
✅ Re-running scan does not create duplicate tags  

---

## Logs to Watch in ./scripts/wr dev Console
...
```bash
# Kill any running ./scripts/wr dev processes
