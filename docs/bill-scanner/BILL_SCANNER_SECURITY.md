...
- Set in `worker/./scripts/wr.toml` under `[env.production]` and `[env.development]`
- Or export before `./scripts/wr dev`: `export BILL_SCANNER_ENABLED=true`
...
### Local Testing (`./scripts/wr dev --local`)
...
```bash
# Start ./scripts/wr WITHOUT OPENAI_API_KEY
...
- [ ] `BILL_SCANNER_ENABLED=true` is exported before `./scripts/wr dev`
- [ ] `OPENAI_API_KEY` is set to a valid OpenAI key
- [ ] curl uses `http://127.0.0.1:8787` (not `localhost` or another hostname)
- [ ] ./scripts/wr dev is running in foreground (see logs)
