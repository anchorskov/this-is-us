...
**Important:** Pages deployment does NOT deploy the Worker. Worker must be deployed separately via `./scripts/wr deploy`.
...
- [ ] Worker deploy successful: `./scripts/wr deploy --env production` exits 0
...
2. Check Worker Tail for errors: `./scripts/wr tail --env production`
