...
───────────
cd /home/anchor/projects/this-is-us/worker && ./scripts/wr deploy --env production
...
───────────
cd /home/anchor/projects/this-is-us/worker && ./scripts/wr d1 execute EVENTS_DB --env production --remote "PRAGMA table_info('podcast_uploads');"
...
Before Deploy:
  - Run exactly: cd worker && ./scripts/wr deploy --env production
  - NOT: npm run deploy (that's for Pages)
  - NOT: ./scripts/wr deploy --env development (wrong env)
...
If Tests Fail:
  1. Check Worker Tail: ./scripts/wr tail --env production
  2. Look for diagnostic logs: [podcast/summary] ...
  3. Check if alternate path works (if yes, primary route issue)
  4. Force redeploy: ./scripts/wr deploy --env production --force
  5. Wait 60 seconds for Cloudflare cache refresh

Rollback If Needed:
  git revert HEAD~1 && git push && cd worker && ./scripts/wr deploy --env production
...
Deploy:     Ready (one ./scripts/wr command from worker/ dir)
Rollback:   Ready (one git command)
Docs:       Complete (4 comprehensive guides)

Status:     ✅ ALL CHANGES COMPLETE AND READY TO DEPLOY

Next Action: Run "./scripts/wr deploy --env production" from worker/ directory
